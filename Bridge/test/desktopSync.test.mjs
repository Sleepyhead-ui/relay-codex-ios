...
test("enhanced desktop sync sends a renderer reload over localhost CDP", async (t) => {
  ...
  const server = createServer((request, response) => {
    if (request.url === "/json/version") {
      ...
      response.end(JSON.stringify({ Browser: "Codex" }));
      return;
    }
...
test("enhanced desktop sync can use a dynamically discovered CDP port", async (t) => {
  let received;
  const server = createServer((request, response) => {
    if (request.url === "/json/version") {
      response.writeHead(200, { "content-type": "application/json" });
    ...
    response.end(JSON.stringify([{
      ...
      title: "Codex",
      url: "app://-/index.html",
      webSocketDebuggerUrl: `ws://127.0.0.1:${address.port}/devtools/page/relay-discovery-test`,
    }]));
  });
  const webSockets = new WebSocketServer({ noServer: true });
  server.on("upgrade", (request, socket, head) => {
  ...
  const port = server.address().port;
  const sync = new DesktopSync(true, 1, undefined, () => {}, () => {}, async () => [port]);
  assert.equal(await sync.reloadDesktopRenderer(), true);
  await new Promise((resolve) => setTimeout(resolve, 50));
  assert.deepEqual(received, { id: 1, method: "Page.reload", params: { ignoreCache: false } });
});
